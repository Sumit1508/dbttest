{{ config(
    ...
    post_hook=[ 
        ...
        BEGIN
            ...
            -- Log the outcome of the MERGE operation
            INSERT INTO DEV.ACCEL_LOGGINGDB.MERGE_OPERATION_LOGS (operation_id, status, rows_affected, error_message)
            SELECT 'MERGE_' || TO_CHAR(CURRENT_TIMESTAMP(), 'YYYYMMDD_HH24MISS'),
                   'SUCCESS',
                   (SELECT COUNT(*) FROM DEV.ACCEL_BI_BR.DWH_Package_DBTPOC),
                   NULL;
        EXCEPTION
            WHEN OTHERS THEN
                -- Log any error if the MERGE fails
                INSERT INTO DEV.ACCEL_LOGGINGDB.MERGE_OPERATION_LOGS (operation_id, status, rows_affected, error_message)
                SELECT 'MERGE_' || TO_CHAR(CURRENT_TIMESTAMP(), 'YYYYMMDD_HH24MISS'),
                       'ERROR',
                       0,
